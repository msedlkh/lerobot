...
def run_so100(robot_config):
    ...
    # robot = LeKiwi(motors_bus) #this sets the wheels for the lekiwi, but since so100 doesn't have wheels, 
    # we don't need to set the wheels for the so100
    ...
    try:
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    if "raw_velocity" in data:
                        ...
                        # robot.set_velocity(command_speeds) # not needed for so100
                        last_cmd_time = time.time()
                except Exception as e:
            ...
            # current_velocity = robot.read_velocity() # not needed for so100
        ...
        print("Shutting down so100 server.")
    finally:
        stop_event.set()
        cam_thread.join()
        # robot.stop() # not needed for so100
